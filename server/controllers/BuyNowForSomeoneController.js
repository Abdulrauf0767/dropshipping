const BuynowforSomeone = require('../models/BuyNowForSomeone.Model');
const UserModel = require('../models/User.Model');
const ProductModel = require('../models/Product.Model');
const NotificationModel = require('../models/Notification.Model');

class BuyNowForSomeoneController {
  ...
  async createOrder(req, res) {
    try {
      ...
      const order = await BuynowforSomeone.create({
        user: user._id,
  ...
  async getOrders(req, res) {
    try {
      const userId = req.user._id;
      const orders = await BuynowforSomeone.find({ user: userId }).populate('products', 'name price image');
  ...
  async getOrderById(req, res) {
    try {
      ...
      const order = await BuynowforSomeone.findById(orderId).populate('products', 'name price image');
      if (!order) {
  ...
  async deleteOrder(req, res) {
    try {
      const orderId = req.params.id;
      const order = await BuynowforSomeone.findByIdAndDelete(orderId);
      if (!order) {
        return res.status(404).json({ message: 'Order not found' });
      }
  ...
  async updateOrder(req, res) {
    try {
      ...
      const updatedOrder = await BuynowforSomeone.findByIdAndUpdate(orderId, updateData, { new: true });
      if (!updatedOrder) {
        return res.status(404).json({ message: 'Order not found' });
      }
  ...
  async getAllOrders(req, res) {
    try {
      const orders = await BuynowforSomeone.find({})
  ...
  async updateOrderStatus(req, res) {
    try {
      ...
      const updatedOrder = await BuynowforSomeone.findByIdAndUpdate(orderId, { orderStatus }, { new: true });
      if (!updatedOrder) {
        return res.status(404).json({ message: 'Order not found' });
      }
  ...
  async updatePaymentStatus(req, res) {
    try {
      ...
      const updatedOrder = await BuynowforSomeone.findByIdAndUpdate(orderId, { paymentStatus }, { new: true });
      if (!updatedOrder) {
        return res.status(404).json({ message: 'Order not found' });
      }
...
module.exports = new BuyNowForSomeoneController();
